...
namespace MultiPrecision {
    ...
    public sealed partial class MultiPrecision<N> {
        ...
        public static MultiPrecision<N> Gamma(MultiPrecision<N> x) {
            if(x.Sign == Sign.Plus) {
                if(x.Exponent >= Consts.LogGamma.ExponentThreshold) { 
                    MultiPrecision<N> y = Exp(LogGammaSterlingApprox(x));

                    return RoundMantissa(y, Math.Min(Bits - 12, MultiPrecision<Pow2.N8>.Bits - 12));
                }
                else {
                    MultiPrecision<N> x_int = Floor(x), x_frac = x - x_int, z = x_frac + Integer(Consts.LogGamma.ApproxThreshold);
                    MultiPrecision<N> w = Exp(LogGammaSterlingApprox(z));
        ...
        internal static MultiPrecision<N> LogGammaSterlingApprox(MultiPrecision<N> x) {
            ...
            MultiPrecision<N> w = (x + Ldexp(MinusOne, -1)) * Log(x) - x + Consts.LogGamma.C0;
